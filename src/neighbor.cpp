...
void Neighbor::morph_unique()
{
  ...
  for (int i = 0; i < nrequest; i++) {
    ...
    if (irq->sve) irq->omp = 0;

    // avoid flagging a neighbor list as both KOKKOS and INTEL or OPENMP

    if (irq->kokkos_host || irq->kokkos_device) irq->omp = irq->intel = irq->sve = 0;
  }
}
  ...
  for (int i = 0; i < npclass; i++) {
    ...
    if (rq->copy) {
      ...
      if (rq->trim) {
        ...
        if (!rq->sve != !(mask & NP_SVE)) continue;
      }
    ...
    if (!rq->cluster != !(mask & NP_CLUSTER)) continue;
...
NeighRequest *Neighbor::add_request(Pair *requestor, int flags)
{
  ...
  if (requestor->suffix_flag & Suffix::SVE) {
    req->sve = 1;
    req->omp = 0;
  }
  return req;
