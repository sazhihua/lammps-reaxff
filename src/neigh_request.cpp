...
NeighRequest::NeighRequest(LAMMPS *_lmp) : Pointers(_lmp)
{
  ...
  // default is no KUNPENG SVE neighbor list build
...
void NeighRequest::apply_flags(int flags)
{
  ...
  if (flags & REQ_CLUSTER)         { cluster = 1; }
  // clang-format on
}
...
void NeighRequest::disable_cluster()
{
  cluster = 0;
}