...
namespace ReaxFFSVE {
  ...
  #pragma omp parallel default(shared) reduction(+: total_Ebond)
  {
    ...
    double *CdDelta_local = workspace->CdDeltaTmp[tid];
    memset(CdDelta_local, 0.0, sizeof(double) * Natoms); // first use, so zeroed
      ...
      for (pj = start_i; pj < end_i; ++pj) 
        ...
        if (bo_ij->BO >= 1.00) {
          if (gp37 == 2 ||
            ...
            CdDelta_local[i] += decobdboua;
            CdDelta_local[j] += decobdboub;
          }
        }
      }
    }
    ...
    if (system->pair_ptr->eflag_either)
      system->pair_ptr->eng_vdwl += total_Ebond;
  }
}
