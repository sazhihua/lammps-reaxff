...
FixReaxFFSVE::FixReaxFFSVE(LAMMPS *lmp,int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
double FixReaxFFSVE::memory_usage()
{
  int nmax = atom->nmax;
...
void FixReaxFFSVE::grow_arrays(int nmax)
{
...
void FixReaxFFSVE::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixReaxFFSVE::pack_exchange(int i, double *buf)
{
  buf[0] = num_bonds[i];
  buf[1] = num_hbonds[i];
  return 2;
...
int FixReaxFFSVE::unpack_exchange(int nlocal, double *buf)
{
...
int FixReaxFFSVE::pack_forward_comm(int n, int *list, double *buf,
...
void FixReaxFFSVE::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
