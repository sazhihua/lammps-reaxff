...
#include "fix_qeq_rel_reaxff_sve.h"
...
FixQEqRelReaxFFSVE::FixQEqRelReaxFFSVE(LAMMPS *lmp, int narg, char **arg) : FixQtpieReaxFFSVE(lmp, narg, arg)
{
}

/* ---------------------------------------------------------------------- */

void FixQEqRelReaxFFSVE::calc_chi_eff()
{
  ...
  if (efield) {
    if (efield->varflag != FixEfieldSVE::CONSTANT) efield->update_efield_variables();
