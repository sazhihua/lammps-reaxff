...
namespace ReaxFFSVE {
  ...
  double total_e_lp = 0.0;
  double total_e_ov = 0.0;
  double total_e_un = 0.0;
  double total_e_vdwl = 0.0;

  #pragma omp parallel num_threads(nthr) reduction(+: total_e_lp, total_e_ov, total_e_un, total_e_vdwl)
  {
    ...
    double *CdDelta_local = workspace->CdDeltaTmp[ithr];
    ...
    for (i = 0; i < system->n; ++i) {
      ...
      numbonds = 0;
      int start_pj = Start_Index(i, bonds);
      int end_pj = End_Index(i, bonds);
      numbonds = end_pj - start_pj;
      e_lp = 0.0;

      /* calculate the energy */
      if (numbonds > 0 || control->enobondsflag)
        // data->my_en.e_lp += e_lp = p_lp2 * workspace->Delta_lp[i] * inv_expvd2;
        // eng_virial0[0] += e_lp = p_lp2 * workspace->Delta_lp[i] * inv_expvd2;
        total_e_lp += e_lp = p_lp2 * workspace->Delta_lp[i] * inv_expvd2;
      ...
      if (numbonds > 0 || control->enobondsflag)
        ...
        CdDelta_local[i] += CElp;  // lp - 1st term
      ...
      if (p_lp3 > 0.001 && !strcmp(system->reax_param.sbp[type_i].name, "C"))
        for (pj = start_pj; pj < end_pj; ++pj) {
          ...
          if (vov3 > 3.) {
            ...
            // eng_virial0[0] += e_lph = p_lp3 * SQR(vov3 - 3.0);
            total_e_lp += e_lph = p_lp3 * SQR(vov3 - 3.0);
    ...
    for (i = 0; i < system->n; ++i) {
      ...
      // eng_virial0[2] += e_ov = sum_ovun1 * CEover1;
      total_e_ov += e_ov = sum_ovun1 * CEover1;
      ...
      for (pj = start_pj; pj < end_pj; ++pj) {
        ...
        CdDelta_local[j] += CEover4 * (1.0 - dfvl*workspace->dDelta_lp[j]) *
        ...
        CdDelta_local[j] += CEunder4 * (1.0 - dfvl*workspace->dDelta_lp[j]) *
    ...
    data->my_en.e_lp += total_e_lp;
    data->my_en.e_ov += total_e_ov;
    data->my_en.e_un += total_e_un;
    if (system->pair_ptr->eflag_either) 
      system->pair_ptr->eng_vdwl += total_e_vdwl;
