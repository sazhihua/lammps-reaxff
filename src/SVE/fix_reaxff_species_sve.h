...
#ifdef FIX_CLASS
// clang-format off
FixStyle(reaxff/species/sve,FixReaxFFSpeciesSVE);
// clang-format on
#else

#ifndef LMP_FIX_REAXC_SPECIES_SVE_H
#define LMP_FIX_REAXC_SPECIES_SVE_H
...
class FixReaxFFSpeciesSVE : public Fix {
 public:
  FixReaxFFSpeciesSVE(class LAMMPS *, int, char **);
  ~FixReaxFFSpeciesSVE() override;
 ...
 protected:
  ...
  std::vector<std::string> eletype;     // list of ReaxFFSVE elements of length ntypes
  std::vector<std::string> ueletype;    // list of unique elements, of quantity nutypes

  void Output_ReaxFF_Bonds(bigint, FILE *);
  AtomCoordSVE chAnchor(AtomCoordSVE, AtomCoordSVE);
