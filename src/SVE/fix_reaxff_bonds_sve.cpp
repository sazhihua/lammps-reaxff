...
#include "fix_reaxff_bonds_sve.h"
...
FixReaxFFBondsSVE::FixReaxFFBondsSVE(LAMMPS *lmp, int narg, char **arg) :
...
FixReaxFFBondsSVE::~FixReaxFFBondsSVE()
{
  destroy();
...
void FixReaxFFBondsSVE::setup(int /*vflag*/)
{
...
void FixReaxFFBondsSVE::end_of_step()
{
  Output_ReaxFF_Bonds();
}
...
void FixReaxFFBondsSVE::Output_ReaxFF_Bonds()
...
int FixReaxFFBondsSVE::FindBond()
{
  int *ilist, i, ii, inum;
  int j, pj, nj;
  tagint jtag;
...
void FixReaxFFBondsSVE::PassBuffer(double *buf, int &nbuf_local)
{
  int i, j, k, numbonds;
  int nlocal = atom->nlocal;
...
void FixReaxFFBondsSVE::RecvBuffer(double *buf, int nbuf, int nbuf_local,
...
int FixReaxFFBondsSVE::nint(const double &r)
{
  int i = 0;
...
void FixReaxFFBondsSVE::destroy()
{
  memory->destroy(abo);
...
void FixReaxFFBondsSVE::allocate()
{
...
int FixReaxFFBondsSVE::modify_param(int narg, char **arg)
{
...
double FixReaxFFBondsSVE::memory_usage()
{
  double bytes;
