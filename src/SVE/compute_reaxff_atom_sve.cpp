...
#include "compute_reaxff_atom_sve.h"
...
ComputeReaxFFAtomSVE::ComputeReaxFFAtomSVE(LAMMPS *lmp, int narg, char **arg) :
...
ComputeReaxFFAtomSVE::~ComputeReaxFFAtomSVE()
{
...
void ComputeReaxFFAtomSVE::init()
{
  if (lmp->suffix_enable) {
    if (lmp->suffix)
      reaxff = dynamic_cast<PairReaxFFSVE *>(force->pair_match(fmt::format("^reax../{}", lmp->suffix), 0, nsub));
    if (!reaxff && lmp->suffix2)
      reaxff = dynamic_cast<PairReaxFFSVE *>(force->pair_match(fmt::format("^reax../{}", lmp->suffix2), 0, nsub));
  }

  if (!reaxff) reaxff = dynamic_cast<PairReaxFFSVE *>(force->pair_match("^reax..", 0, nsub));
...
int ComputeReaxFFAtomSVE::FindBond()
{
  int *ilist, i, ii, inum;
  int j, pj, nj;
  tagint jtag;
...
void ComputeReaxFFAtomSVE::compute_bonds()
{
...
void ComputeReaxFFAtomSVE::compute_local()
{
...
void ComputeReaxFFAtomSVE::compute_peratom()
{
...
double ComputeReaxFFAtomSVE::memory_usage()
{
