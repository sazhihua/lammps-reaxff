...
FixEfieldSVE::FixEfieldSVE(LAMMPS *lmp, int narg, char **arg) :
...
void FixEfieldSVE::setup(int vflag)
{
  if (utils::strmatch(update->integrate_style, "^respa")) {
...
void FixEfieldSVE::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixEfieldSVE::post_force(int vflag)
{
  double **f = atom->f;
  double *q = atom->q;
  int *mask = atom->mask;
...
void FixEfieldSVE::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixEfieldSVE::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixEfieldSVE::memory_usage()
{
  double bytes = 0.0;
...
double FixEfieldSVE::compute_scalar()
{
  if (force_flag == 0) {
...
double FixEfieldSVE::compute_vector(int n)
{
  if (force_flag == 0) {
    MPI_Allreduce(fsum, fsum_all, 4, MPI_DOUBLE, MPI_SUM, world);
    force_flag = 1;
...
void FixEfieldSVE::update_efield_variables()
{
