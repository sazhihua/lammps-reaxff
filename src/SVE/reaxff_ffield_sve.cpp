...
#define CHECK_COLUMNS(want)                                                          \
  if (values.count() < static_cast<std::size_t>(want))                               \
    ...
    if (control->me == 0) {
      ...
      if (!fp)
        error->one(FLERR,"The ReaxFFSVE parameter file {} cannot be opened: {}",
                   filename, getsyserror());
      LAMMPS_NS::TextFileReader reader(fp, "ReaxFFSVE parameter");
      ...
      try {
        ...
        if (strmatch(line, "^\\s*[0-9]+\\s+!.*general parameters.*"))
          THROW_ERROR("First line of ReaxFFSVE potential file must be a comment or empty");
        ++lineno;
        ...
        for (i = 0; i < ntypes; ++i) {
          ...
          if (lgflag) {
            ...
            if (!values.matches("^\\s*\\f+\\s*"))
              THROW_ERROR("ReaxFFSVE potential file is not compatible with 'lgvdw yes'");
        ...
        if (!thisline) throw EOFException("ReaxFFSVE parameter file has no hydrogen bond parameters");
    ...
    // GPTLstart("bop preprocess");

    for (int i = 0; i < reax->num_atom_types; i ++) {
      for (int j = 0; j < reax->num_atom_types; j ++) {
        auto sbp_i = &(reax->sbp[i]);
        auto sbp_j = &(reax->sbp[j]);
        auto twbp = &(reax->tbp[i][j]);
        auto boR = [&](double d){
          double BO_s, BO_pi, BO_pi2, C12, C34, C56;
          ...
          return BO_s + BO_pi + BO_pi2;
        };
        double rmin = 1, rmax = control->bond_cut;
        do {
          double rmid = (rmin+rmax)*0.5;
          double midbo = boR(rmid);
          if(midbo > control->bo_cut) rmin = rmid;
          else rmax = rmid;
        } while (rmax - rmin > 1e-8);
        twbp->r_bocut = rmax;
      }
    }
    // GPTLstop("bop preprocess");
  }
